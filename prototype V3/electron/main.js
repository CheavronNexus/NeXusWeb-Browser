...
function createTab(url) {
  ...
  const initialTitle = targetUrl === 'nexusweb://home' ? 'Home' : 'New Tab'
  ...
  // Audio Playback Tracking — only triggers when actual sound is audible (ignores muted hover previews)
  view.webContents.on('audio-state-changed', (event, audible) => {
    const tab = tabs.get(tabId)
    if (tab && tab.isPlayingAudio !== audible) {
...
function resolveUrl(input) {
  ...
  if (str.startsWith('http://') || str.startsWith('https://') || str.startsWith('file://') || str.startsWith('devtools://')) {
    return str
  }
  if (/^\d{1,5}$/.test(str)) {
