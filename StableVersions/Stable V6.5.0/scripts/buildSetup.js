...
console.log('=================================================================')
console.log('   Chevron Nexus Software — NeXusWeb V6 Setup & Updater Builder   ')
console.log('=================================================================')

const rootDir = path.join(__dirname, '..')
const distElectronDir = path.join(rootDir, 'dist-electron')
const appDir = path.join(distElectronDir, 'NeXusWeb-V6-win32-x64')
const payloadZip = path.join(distElectronDir, 'app-payload.zip')
const setupExeOut = path.join(distElectronDir, 'NeXusWeb-Setup-v6.5.0.exe')
...
try { execSync('taskkill /F /IM "NeXusWeb-Setup-v6.5.0.exe" /T 2>nul', { stdio: 'ignore' }) } catch (e) {}
try { execSync('taskkill /F /IM "NeXusWeb-Setup-v6.0.0.exe" /T 2>nul', { stdio: 'ignore' }) } catch (e) {}
...
console.log('\n[2/5] Packaging NeXusWeb-V6 Standalone Binary (Electron 28.3.3 x64)...')
execSync('npx electron-packager . "NeXusWeb-V6" --platform=win32 --arch=x64 --icon="src/assets/app.ico" --electron-version=28.3.3 --out="dist-electron" --overwrite --asar --ignore="dist-electron|\\.git|\\.vite"', { cwd: rootDir, stdio: 'inherit' })
...
console.log('\n[4/5] Compiling Standalone Windows Setup & Upgrader (NeXusWeb-Setup-v6.5.0.exe)...')
...
const manifest = {
  ...
  executable: 'NeXusWeb-V6.exe',
  setupFile: 'NeXusWeb-Setup-v6.5.0.exe',
...
console.log('\n=================================================================')
console.log('   SUCCESS: NeXusWeb V6.5 Setup & Updater Built Successfully!    ')
console.log('   Standalone Setup: dist-electron/NeXusWeb-Setup-v6.5.0.exe      ')
console.log('   Standard Alias:   dist-electron/setup.exe                      ')
console.log('   Unpacked App:     dist-electron/NeXusWeb-V6-win32-x64          ')
