...
function createWindow(initialUrl = null, isPrivate = false) {
  ...
  vpnEngine.start().catch(e => console.warn('[NeXus VPN] Start warning:', e.message))
...
ipcMain.handle('proxy-set-config', async (_, config) => {
  try {
    if (!config || config.mode === 'direct') {
      await vpnEngine.setRegion('direct')
      await session.defaultSession.setProxy({ mode: 'direct' })
      return { success: true, config: vpnEngine.getConfig() }
    }
    ...
    if (config.mode === 'fixed_servers' && config.proxyRules) {
      ...
      return { success: true, config: { mode: 'proxy', region: 'custom', proxyRules: config.proxyRules } }
    }
    ...
    if (config.mode === 'pac' && config.pacScript) {
      ...
      return { success: true, config: { mode: 'pac', pacFileUrl } }
    }
...
ipcMain.handle('proxy-check-ip', async () => {
  try {
    ...
    return {
      ...
      country: data.country_name || 'Anonymous',
      city: data.city || '',
      countryCode: data.country_code || 'UN',
      org: data.org || 'Secure Tunnel',
      latency: `${latency}ms`,
    }
  } catch (e) {
    return {
