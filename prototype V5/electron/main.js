...
function repositionAllViewsForWindow(state) {
  ...
  const TOP_OFFSET = (state.isSplitView ? 134 : 100) + HUD_OFFSET
  ...
  // If Reader Mode is active or full-screen Settings/Modes Info is open, do NOT attach BrowserView so DOM overlay is 100% visible!
  if (state.isReaderModeOpen || state.activeDrawer === 'settings' || state.isModesInfoOpen) {
    return
  }
...
ipcMain.handle('split-view-get-state', (event) => {
  const state = getWindowStateFromSender(event.sender)
  return { enabled: state?.isSplitView || false, splitTabId: state?.splitTabId, activeTabId: state?.activeTabId }
})
...
const REGIONAL_VPN_PROXIES = {
  direct: { mode: 'direct', region: 'direct' },
  nl: { mode: 'fixed_servers', region: 'nl', proxyRules: 'http=185.220.101.5:80;https=185.220.101.5:80;direct://' },
  us: { mode: 'fixed_servers', region: 'us', proxyRules: 'http=198.199.86.11:8080;https=198.199.86.11:8080;direct://' },
  sg: { mode: 'fixed_servers', region: 'sg', proxyRules: 'http=139.59.245.195:8080;https=139.59.245.195:8080;direct://' },
  uk: { mode: 'fixed_servers', region: 'uk', proxyRules: 'http=51.89.255.67:80;https=51.89.255.67:80;direct://' },
}

let currentProxyConfig = { mode: 'direct', region: 'direct' }
ipcMain.handle('proxy-get-config', () => currentProxyConfig)
ipcMain.handle('proxy-set-mode', async (_, { mode, region = 'direct' }) => {
  try {
    if (mode === 'direct' || region === 'direct') {
      currentProxyConfig = { mode: 'direct', region: 'direct' }
      await session.defaultSession.setProxy({ mode: 'direct' })
      return { success: true, config: currentProxyConfig }
    }
    const conf = REGIONAL_VPN_PROXIES[region] || { mode: 'fixed_servers', region, proxyRules: 'direct://' }
    currentProxyConfig = { mode: 'proxy', region: conf.region, proxyRules: conf.proxyRules }
