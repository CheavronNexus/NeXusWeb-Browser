...
import { Shield, Lock, Radio, Zap, Globe, Github, BookOpen, MessageSquare, Video, Cpu, Server, Code2, Search, ArrowRight } from 'lucide-react'
    ...
    <div className="home-page" style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      <div className="home-section">
        <div className="home-section-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span>Quick Launch Port / URL</span>
        </div>
      ...
      <div className="home-section">
        <div className="home-section-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span>Developer & Web Quick Links</span>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {[
            ...
            { label: 'Stack Overflow',              url: 'https://stackoverflow.com', icon: MessageSquare },
            { label: 'YouTube',                     url: 'https://youtube.com', icon: Video },
            { label: 'Vite (localhost:5173)',        url: 'http://localhost:5173', icon: Zap },
            { label: 'React (localhost:3000)',      url: 'http://localhost:3000', icon: Code2 },
            { label: 'Flask (localhost:5000)',      url: 'http://localhost:5000', icon: Server },
            { label: 'Django (localhost:8000)',     url: 'http://localhost:8000', icon: Cpu },
          ].map(({ label, url, icon: IconComp }) => (
            <button
              key={url}
              className="scan-btn"
              onClick={() => onPortClick(url)}
            ...
            >
              <IconComp size={13} style={{ color: 'var(--accent-primary)' }} />
              <span>{label}</span>
            </button>
          ))}
        </div>
      ...
      <div className="home-section">
        <div className="home-section-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span>Network & Privacy Modes</span>
        </div>
      ...
      <div style={{ paddingTop: 24, borderTop: '1px solid var(--border-subtle)', color: 'var(--text-muted)', fontSize: 11, display: 'flex', justifyContent: 'space-between' }}>
        <span>NeXusWeb v4.0.0 · Localhost, LAN & Privacy Web Browser</span>
